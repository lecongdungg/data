...
func main() {
	...
	flag.BoolVar(&flagPro, "pro", flagPro, "Phiên bản Pro?")
	flag.BoolVar(&flagRefresh, "refresh", flagRefresh, "Làm mới Dữ liệu")
	flag.BoolVar(&flagVerbose, "verbose", flagVerbose, "Ghi log chi tiết?")
	flag.StringVar(&flagFrontend, "f", flagFrontend, "-f frontend-domains (ví dụ: -f cdn.com,cdn.com:443)")
	flag.StringVar(&flagWhitelist, "w", flagWhitelist, "-w whitelist-request (ví dụ: -w akamai.net:80)")
	flag.IntVar(&config.Inject.MeekType, "mt", config.Inject.MeekType, "-mt loại meek (0 và 1 cho fastly)")
	flag.IntVar(&config.PsiphonCore, "c", config.PsiphonCore, "-c core (ví dụ: -c 4) (1 cho phiên bản Pro)")
	flag.StringVar(&config.Psiphon.Region, "r", config.Psiphon.Region, "-r khu vực (ví dụ: -r sg)")
	flag.IntVar(&config.Psiphon.Tunnel, "t", config.Psiphon.Tunnel, "-t tunnel (ví dụ: -t 4) (1 cho phiên bản Reconnect)")
	flag.IntVar(&config.Psiphon.TunnelWorkers, "tw", config.Psiphon.TunnelWorkers, "-tw tunnel-workers (ví dụ: -tw 6) (8 cho phiên bản Pro)")
	flag.IntVar(&config.Psiphon.KuotaDataLimit, "l", config.Psiphon.KuotaDataLimit, "-l giới hạn (theo MB) (ví dụ: -l 4) (0 cho phiên bản Pro (không giới hạn))")
	flag.Parse()
	...
	liblog.LogInfo("Domain Fronting đang chạy trên cổng "+Inject.Config.Port, "INFO", liblog.Colors["G1"])
	liblog.LogInfo("Proxy Rotator đang chạy trên cổng "+ProxyRotator.Config.Port, "INFO", liblog.Colors["G1"])
